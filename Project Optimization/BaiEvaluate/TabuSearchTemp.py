...
def neighbors_generate(ind, precedence, available_team, num_neighbors=10, attempts=5, mutation_rate=0.5):
    ...
    for _ in range(num_neighbors):
        neighbor = get_neighbor(ind, precedence, available_team, attempts, mutation_rate)
...
for gen in range(1, max_iter + 1):
    ...
    neighbors = neighbors_generate(curr, precedence, available_team, 10)
    best_neighbor = None
    ...
    if gen % max_converge == 0:
        print(f"Gen: {gen}, best_fitness: {(num_task - f_best[0], f_best[1], f_best[2])}, current_fitness: {(num_task - best_neighbor_fit[0], best_neighbor_fit[1], best_neighbor_fit[2])}")

print('------------------------')
print(f"Tabu Search")
print(f"Best_fitness: {(num_task - f_best[0], f_best[1], f_best[2])}")
