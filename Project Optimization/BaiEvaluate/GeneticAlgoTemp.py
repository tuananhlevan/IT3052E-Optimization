...
def mutate(ind, precedence, available_team, attempts=5, mutation_rate=0.5):
...
for gen in range(1, max_generations + 1):
    ...
    if (gen % max_converge) == 0:
        ...
        print(f"Current generation: {gen}, best_fitness: {(num_task - best_fitness[0], best_fitness[1], best_fitness[2])}")

best_chromosome = population[0]
best_fitness = calc_fitness(best_chromosome, num_task, cost, task_duration, s, precedence_graph)
print('----------------------------------')
print(f"Genetic Algorithm")
print(f"Best fitness: {(num_task - best_fitness[0], best_fitness[1], best_fitness[2])}")
